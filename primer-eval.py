...
class Primer3Validator:
    ...
    def _validate_sequence(self, seq: str, name: str = "Primer") -> str:
        ...
        if len(seq) >= 60:
            raise ValueError(f"{name} sequence must be < 60 bp for reliable thermodynamic calculations (current: {len(seq)} bp)")
        if len(seq) < 15:
            raise ValueError(f"{name} sequence should be at least 15 bp for PCR applications (current: {len(seq)} bp)")
    ...
    def _safe_thermo_calculation(self, func, *args, **kwargs) -> Tuple[float, bool]:
        try:
            ...
            if "sequence too long" in str(e).lower():
                raise ValueError(f"Sequence too long for thermodynamic analysis: {args[0]}")
            return 0.0, False
    ...
    def _analyze_single_primer(self, seq: str) -> PrimerAnalysis:
        ...
        try:
            ...
            raise RuntimeError(f"Tm calculation failed for sequence '{seq}': {str(e)}")
        ...
        # harpin
        hairpin_dg, hairpin_found = self._safe_thermo_calculation(calc_hairpin, seq, **thermo_params)
        
        # homodimers
        homodimer_dg, homodimer_found = self._safe_thermo_calculation(calc_homodimer, seq, **thermo_params)
    ...
    def analyze_primer_pair(self, forward_seq: str, reverse_seq: str) -> PrimerPairAnalysis:
        ...
        fwd_to_rev_risk, fwd_dg, fwd_detail = self._check_three_prime_stability(fwd_seq, rev_seq)
        rev_to_fwd_risk, rev_dg, rev_detail = self._check_three_prime_stability(rev_seq, fwd_seq)
    ...
    def generate_report(self, analysis: PrimerPairAnalysis) -> str:
        ...
        report_lines.append(f"Hairpin ΔG:  {analysis.forward.hairpin_dg:.2f} kcal/mol")
        report_lines.append(f"Homodimer ΔG: {analysis.forward.homodimer_dg:.2f} kcal/mol")
        ...
        report_lines.append(f"Hairpin ΔG:  {analysis.reverse.hairpin_dg:.2f} kcal/mol")
        report_lines.append(f"Homodimer ΔG: {analysis.reverse.homodimer_dg:.2f} kcal/mol")
        
        report_lines.append("\nPRIMER PAIR")
        report_lines.append("-" * 30)
        report_lines.append(f"Tm difference:    {analysis.tm_difference:.2f} °C")
        report_lines.append(f"Heterodimer ΔG:   {analysis.heterodimer_dg:.2f} kcal/mol")
        ...
        if analysis.three_prime_risk:
            ...
            report_lines.append("3'-end stability: No significant risks detected")
...
if __name__ == "__main__":
    ...
    argparser = ArgumentParser("primer-eval", description="A simple tool to evaluation primer properites")
    argparser.add_argument("forward_primer", help="Sequence of your forward primer (5\'-3\')")
    argparser.add_argument("reverse_primer", help="Sequence of your reverse primer (5\'-3\')")
    ...
    try:
        analysis = validator.analyze_primer_pair(forward_primer, reverse_primer)
        ...
        print(f"✅ JSON result available with {len(json_result['warnings'])} warnings")
